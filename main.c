...
---Patrick Limber (PWL) Changes Oct-Nov-Dec 2014---
1. - Sea cliffs
		- added new variables for subaerial sea cliff height for both hard & soft (i.e., 'fast' and 'slow') rock types

2. - Sea cliff retreat
		- added abrasion to hasten sea cliff retreat when the beach is near an optimal beach width ('wcrit'). 
			Amount that abrasion hastens retreat is controlled by variable 'N' -- it is a multiplier of the baseline rock retreat rate.
...
double SlowWeatherCoeff = NormalWeatheringRate * 0;	/* Weathering rate of slow rock e.g. 0.5 = slow weathering, 1/2 as fast LMV */
double FastWeatherCoeff = NormalWeatheringRate * 1;	/* Weathering rate of fast rock e.g. 2 = fast weathering, 2 times faster than normal LMV */
...
char   VaryCliffHeight = 'y';  /* Vary cliff height for different rock weathering rates? PWL */
char    Abrasion = 'y';         /* Turn abrasion on? If yes, then rock weathering is maximized at a given amount of sediment cover (wcrit).
...
float   NoWeathering = 50.0;	/* weathering of rock only occurs below this amt of sed cover (vert equiv in meters) LMV */
...
int     ChunkLength = 100;	/* Alongshore length of a chunk of fast or slow weathering rock LMV */
...
double  Topography[Xmax][2*Ymax];  /* Holds topographic data so that rock elevation can be spatially variable. PWL */
float	MinDistToBeach[MaxBeachLength]; /* Distance between rock interface and beach position */
...
void WeatherRock(int j)
	...
	{
	  float WeatheringRatePerYear,AngleFactor,CurrentWeatherCoeff,mslope,wscale,CurrentPercentFine;

	/* Assign maximum weathering rate depending on rock type  */
	CurrentWeatherCoeff = (TypeOfRock[XRock[j]][YRock[j]] == 'f' ? FastWeatherCoeff : SlowWeatherCoeff);

	if(CurrentWeatherCoeff != FastWeatherCoeff && CurrentWeatherCoeff != SlowWeatherCoeff) {
		CurrentWeatherCoeff = (TypeOfRock[XRockBehind[j]][YRockBehind[j]] == 'f' ? FastWeatherCoeff : SlowWeatherCoeff);
	}
	if(CurrentWeatherCoeff != FastWeatherCoeff && CurrentWeatherCoeff != SlowWeatherCoeff) {
	...
	CurrentPercentFine = (TypeOfRock[XRock[j]][YRock[j]] == 'f' ? PercentFineFast : PercentFineSlow);
	if(CurrentPercentFine != PercentFineFast && CurrentPercentFine != PercentFineSlow)
	  CurrentPercentFine = (TypeOfRock[XRockBehind[j]][YRockBehind[j]] == 'f' ? PercentFineFast : PercentFineSlow);
	...
	if ((MinDistToBeach[j]) <= NoWeathering) {  /*think in terms of vertical equivalence */
	  ...
	  if (Abrasion == 'n') {
		WeatheringRatePerYear = AngleFactor * CurrentWeatherCoeff 
			* (exp(-MinDistToBeach[j]));  /*exponential weathering (in meters/yr) based on sed cover*/
	  ...
	  if (Abrasion == 'y') {
	    if (MinDistToBeach[j] <= Wcrit) {
	      ...
	      WeatheringRatePerYear = AngleFactor * (CurrentWeatherCoeff + (mslope*MinDistToBeach[j]));
	      ...
	      WeatheringRatePerYear = AngleFactor * CurrentWeatherCoeff * (exp((MinDistToBeach[j] - Wcrit)/wscale));
		}
	  }
	}
	...
	if(debug12) printf("for %d (%d,%d) rock %c, angfactor %f, weathercoeff %f, mindist %f, rate %f m/yr, frac weathered %f\n",
	...
	if (PercentFullRock[XRock[j]][YRock[j]] >= AmountWeathered[j])
	{
		...
		+ (AmountWeathered[j]*(Topography[XRock[j]][YRock[j]]/DepthShoreface)*CurrentPercentFine);

		if (PercentFullRock[XRock[j]][YRock[j]] < 1.0) AllRock[XRock[j]][YRock[j]] = 'n';
	}

	else if((PercentFullRock[XRock[j]][YRock[j]] 
		  + PercentFullRock[XRockBehind[j]][YRockBehind[j]]) >= AmountWeathered[j])
	{
		...
		PercentFullSand[XRockBehind[j]][YRockBehind[j]] += 
		  (AmountWeathered[j] - PercentFullRock[XRock[j]][YRock[j]])
				  + ((AmountWeathered[j] - PercentFullRock[XRock[j]][YRock[j]])
		 * (Topography[XRockBehind[j]][YRockBehind[j]]/DepthShoreface)*CurrentPercentFine);
		
		PercentFullSand[XRock[j]][YRock[j]] += PercentFullRock[XRock[j]][YRock[j]]
		 + (PercentFullRock[XRock[j]][YRock[j]] * (Topography[XRock[j]][YRock[j]]/DepthShoreface)*CurrentPercentFine);
		   ...
		   so weather all there is in both cells. */
	{
		...
		{
			if(debug12) printf("not enough rock to weather for %d (%d,%d). ",j, XRock[j], YRock[j]);
			if(debug12) printf("this is handle-able but prob. shouldn't happen often.\n");
	...
	for (y = 0; y < 2*Ymax; y++)
		...
		for (x = 0; x <= Xmax; x++)  /*LMV Assign fast and slow weathering portions*/
		{
		 	for (n = 0; n <= 2*NumberChunk; n++)
			{
					...
					{
						if (n%3 == 0 && (!blocks || x > InitialRock - 3)) 
						{/* if even */
							TypeOfRock[x][y] = 's';
							Topography[x][y] = CliffHeightSlow;
						}
