...
extern double CliffHeightSlow; /* Cliff height above sea level for slow
                                  weathering rock PWL */
extern double CliffHeightFast; /* Cliff height above sea level for fast
                                  weathering rock PWL */

/* Overall Shoreface Configuration Arrays - Data file information */
extern char AllBeach[Xmax]
                    [2 * Ymax]; /* Flag indicating of cell is entirely beach */
extern char
    AllRock[Xmax][2 * Ymax]; /* Flag indicating if cell is entirely rock LMV */
extern double PercentFullSand
    [Xmax][2 * Ymax]; /* Fractional amount of cell full of sediment LMV */
extern double PercentFullRock[Xmax][2 * Ymax]; /* Fractional amount of a cell
                                                  full of rock LMV */
extern char TypeOfRock[Xmax]
                      [2 * Ymax]; /* Array to control weathering rates of rock
                                     along the beach LMV */
extern int Age[Xmax][2 * Ymax];   /* Age since cell was deposited */
extern double *
    *Topography; /* Holds cliff heights -- will change through time,
                    eventually... */

extern int
    SinkY[6]; /* a sink is a cell that is routinely emptied (if it is on the
                 beach) */
extern int SinkX[6];
...
extern double ShorelineAngle[MaxBeachLength]; /* Angle between cell and right
                                                 (z+1) neighbor  */
extern double SurroundingAngle
    [MaxBeachLength];               /* Angle between left and right neighbors */
extern char UpWind[MaxBeachLength]; /* Upwind or downwind condition used to
                                       calculate sediment transport */
extern double
    VolumeIn[MaxBeachLength]; /* Sediment volume into ith beach element */
extern double
    VolumeOut[MaxBeachLength]; /* Sediment volume out of ith beach element */
extern double VolumeAcrossBorder
    [MaxBeachLength]; /* Sediment volume across border of ith beach element in
                         m^3/day LMV */
...
extern char
    DirectionAcrossBorder[MaxBeachLength]; /* Flag to indicate if transport
                                              across border is L or R LMV */
extern char
    FlowThroughCell[MaxBeachLength]; /* Is flow through ith cell Left, Right,
                                        Convergent, or Divergent LMV */
extern double DistanceToBeach[MaxBeachLength]; /* Distance in meters from rock
                                                  to beach LMV */
extern double
    MinDistToBeach[MaxBeachLength]; /* From a rock cell j, min distance (in
                                       meters) to closest beach LMV */
extern int
    ClosestBeach[MaxBeachLength]; /* i position of closest rock to beach LMV */
extern double AmountWeathered
    [MaxBeachLength]; /* Amount of rock weathered from rock cell j LMV */
...
extern double
    EvaluateAngle; /* Temporary angle holder for the ConvertAngle function */
...
extern int
    NextX; /* Global variables used to iterate FindNextCell in global array - */
extern int NextY; /*      would've used pointer but wouldn't work */
extern int BehindX;
extern int BehindY;
extern int BehindRockX;
extern int BehindRockY;
extern int NextRockX; /* Global variables used to iterate FindNextRockCell in
                         global array LMV */
extern int NextRockY;
extern int TotalBeachCells; /* Number of cells describing beach at particular
                               iteration */
extern int
    TotalRockCells;    /* Number of cells describing rock at an iteration LMV */
...
extern int
    FindStart; /* Used to tell FindBeach at what Y value to start looking */
extern int FindRockStart; /* Used to tell FindRock at what Y value to start
                             looking LMV */
extern char
    FellOffArray; /* Flag used to determine if accidentally went off array */
extern char
    FellOffRockArray; /* Flag used to determine if accidentally went off rock
                         array LMV */
...
extern double Asym; /* fractional portion of waves coming from positive (left)
                       direction */
extern double Highness; /* .5 = even dist, < .5 high angle domination. NOTE
                           Highness actually determines lowness! */
extern double
    Duration; /* Number of time steps calculations loop at same wave angle */
extern double
    TimeStep; /* days - reflects rate of sediment transport per time step */

extern double
    SlowWeatherCoeff;           /* Weathering rate of slow rock e.g. 0.5 = slow
                                   weathering, 1/2 as fast LMV */
extern double FastWeatherCoeff; /* Weathering rate of fast rock e.g. 2 = fast
                                   weathering, 2 times faster than normal LMV */
extern double PercentFineFast;  /* Percent of fast weathering rock lost because
                                   it is too fine to stay in nearshore LMV */
extern double PercentFineSlow;  /* Percent of slow weathering rock lost because
                                   it is too fine to stay in nearshore LMV */
extern double
    ErosionRatePerYear; /* Amount of erosion to TotalBeachCells in m/year LMV */

extern double coastrotation;    /* Angle (deg) used to align coastline with real
                                   wave climate */
extern double waveheightchange; /* Factor applied to the input wave height; 1 =
                                   no change, 0.5 = half wave height, and 2 =
                                   double wave height */
extern double waveperiodchange; /* Factor applied to the input wave period; 1 =
                                   no change, 0.5 = half wave period, and 2 =
                                   double wave period */
